...
<div class="hdr">
  <div class="hdr-top">
    <div>
      <div class="brand"><span class="live"></span>Chromadata · Track Acquisition Intelligence</div>
...
function renderTable(){const q=document.getElementById('searchInput').value.toLowerCase();const plat=document.getElementById('platformSel').value;const sig=document.getElementById('signalSel').value;let data=[...TRACKS];if(q) data=data.filter(t=>t.title.toLowerCase().includes(q)||t.artist.toLowerCase().includes(q));if(plat!=='all') data=data.filter(t=>t.platform===plat);if(sig!=='all') data=data.filter(t=>t.signal===sig);if(currentPeriod==='rising') data=data.filter(t=>t.momentum>0);if(currentPeriod==='stable') data=data.filter(t=>Math.abs(t.momentum)<=5);if(currentPeriod==='falling') data=data.filter(t=>t.momentum<0);const sortMap={acq:'acqScore',momentum:'momentum',rank:'bestRank',streams:'latestStreams',growth:'growth'};const key=sortMap[currentSort]||'acqScore';const asc=key==='bestRank';data.sort((a,b)=>asc?a[key]-b[key]:b[key]-a[key]);document.getElementById('count-badge').textContent=`${data.length} track${data.length!==1?'s':''}`;const displayData=data;const el=document.getElementById('track-table');el.innerHTML='';displayData.forEach((t,i)=>{const momColor=t.momentum>5?'g':t.momentum<-5?'r':'';const spark=buildSpark(t.spStreams,t.acqColor);el.innerHTML+=`<div class="track-row${t.id===selectedId?' selected':''}" style="grid-template-columns:24px 1fr 50px 70px 80px 100px 75px" onclick="selectTrack(${t.id})"><span class="tr-num">${i+1}</span><div class="tr-info"><div class="tr-title">${t.title}</div><div class="tr-artist">${t.artist} ${t.platform==='cross'?'<span style="color:var(--teal);font-size:8px">✦ cross</span>':''}</div>${spark}</div><span class="tr-val">#${t.bestRank}</span><span class="tr-val">${fmtN(t.latestStreams)}</span><span class="tr-val ${momColor}">${t.momentum>0?'+':''}${t.momentum}%</span><span style="text-align:right;padding-right:12px"><span class="sig ${signalClass(t.signal)}">${signalLabel(t.signal)}</span></span><span class="tr-val a">${t.acqScore}</span></div>`;});}
