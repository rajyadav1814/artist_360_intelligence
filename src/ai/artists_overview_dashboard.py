...
def render_artists_overview(last_run_label: str = "n/a") -> None:
    ...
    if not filtered.empty:
        ...
        modal_df = filtered[modal_cols].copy()
    ...
    def artist_story_html(df: pd.DataFrame) -> str:
        if df.empty:
            return (
...
.radar-shell{height:238px;display:grid;place-items:center;margin-top:2px}.radar{width:min(100%,430px);height:232px;display:block}.radar text{fill:var(--soft);font-size:11px;font-weight:800}.radar-grid circle,.radar-grid line{fill:none;stroke:var(--border);stroke-width:1.2}.radar polygon{fill:rgba(167,139,250,.20);stroke:var(--purple);stroke-width:3;filter:drop-shadow(0 8px 16px rgba(167,139,250,.20))}
