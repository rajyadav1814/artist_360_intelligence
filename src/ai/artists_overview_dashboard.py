...
def render_artists_overview(last_run_label: str = "n/a") -> None:
    ...
    # --- Artist Selection ---
    selected_artist_name = "All Artists"
...
.search-shell{position:relative;margin:0 0 16px;max-width:640px}
.search-border{border-radius:14px;padding:2px;background:linear-gradient(90deg,#3b82f6,#a855f7,#ec4899)}
.search-inner{display:flex;align-items:center;gap:10px;background:var(--panel);border-radius:12px;padding:12px 16px}
.search-icon{flex:0 0 auto;color:var(--soft)}
.search-input{flex:1;min-width:0;border:0;outline:0;background:transparent;color:var(--text);font-size:14px;font-weight:600;font-family:inherit}
.search-input::placeholder{color:var(--soft);font-weight:600}
.search-results{position:absolute;top:calc(100% + 8px);left:0;right:0;background:var(--panel);border:1px solid var(--border);border-radius:12px;box-shadow:var(--shadow);max-height:300px;overflow:auto;z-index:30;display:none}
...
.search-result-item b{color:var(--text);font-size:13px;font-weight:800}
.search-result-sub{font-size:11px;color:var(--soft);font-weight:650;margin-top:2px}
.search-empty{padding:14px;color:var(--muted);font-size:12px;text-align:center}
...
function handleArtistSearch(query) {{
  ...
  const rows = (ARTIST_LEADERBOARD.rows || []).filter(r => String(r.name || '').toLowerCase().includes(q)).slice(0, 8);
  if (!rows.length) {{
    box.innerHTML = '<div class="search-empty">No artists found.</div>';
  }} else {{
    box.innerHTML = rows.map(r => `
      <div class="search-result-item" onclick="selectArtistSearchResult(${{r.position}})">
        <div class="leader-avatar">${{escLeader(initials(r.name))}}</div>
        <div>
          <b>${{escLeader(r.name)}}</b>
          <div class="search-result-sub">Rank #${{fmtLeaderNumber(r.rank)}} · ${{fmtLeaderNumber(r.points)}} pts</div>
        </div>
      </div>
    `).join('');
  }}
...
function selectArtistSearchResult(position) {{
  document.getElementById('globalSearchResults')?.classList.remove('show');
  const input = document.getElementById('globalArtistSearch');
  if (input) input.value = '';
  openArtistLeaderboard();
  openArtistDetail(position);
}}
document.addEventListener('click', event => {{
  const shell = document.querySelector('.search-shell');
  const results = document.getElementById('globalSearchResults');
  if (shell && results && !shell.contains(event.target)) {{
...
document.addEventListener('keydown', event => {{
  if (event.key === 'Escape') {{
    ...
    st_components.html(html, height=950, scrolling=False)