...
<style>
*{box-sizing:border-box}::-webkit-scrollbar{width:6px;height:6px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:var(--border);border-radius:4px}::-webkit-scrollbar-thumb:hover{background:var(--soft)}body{margin:0;font-family:Inter,ui-sans-serif,system-ui;background:transparent}.dash{min-height:800px;padding:18px;color:var(--text);background:transparent}
...
/* Hide any injected Streamlit Cloud viewer badges inside this component iframe */
[data-testid="stHostedBadge"], [data-testid="viewerBadge"], .viewerBadge_container, .viewerBadge_link, div[class*="viewerBadge"], div[class*="_viewerBadge_"], a[class*="_viewerBadge_"], a[href*="streamlit.io/cloud"], div:has(> a[href*="streamlit.io/cloud"]), div:has(> div > a[href*="streamlit.io/cloud"]), div:has(> a[href*="share.streamlit.io"]), div:has(> div > a[href*="share.streamlit.io"]) { display: none !important; visibility: hidden !important; opacity: 0 !important; pointer-events: none !important; }
</style></head><body>
...
function platformPills(platformStr) {{
  return String(platformStr || '').split(', ').filter(Boolean).map(p => {{
    const key = p.toLowerCase();
    const cls = key.includes('itunes') ? 'platform-itunes' : key.includes('spotify') ? 'platform-spotify' : 'platform-mixed';
    return `<span class="platform-pill ${{cls}}">${{escLeader(p)}}</span>`;
  }}).join('');
}}
