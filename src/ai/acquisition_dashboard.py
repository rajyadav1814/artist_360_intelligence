...
.tab-row{display:flex;gap:8px;margin-bottom:10px;background:transparent;border-radius:999px;padding:0;position:sticky;top:0;z-index:3}.tab{flex:1;padding:8px 12px;font-size:12px;font-weight:700;background:var(--bg3);border:1px solid var(--border);border-radius:999px;cursor:pointer;color:var(--t2)}.tab.on{background:var(--bg2);color:var(--t1);border-color:var(--blue);box-shadow:0 0 0 2px rgba(96,165,250,.12)}
...
<div class="d">
  ...
  <div class="tab-row"><button class="tab on" onclick="showTab('top10',this)"><span>Top 10 targets</span></button><button class="tab" onclick="showTab('newartists',this)"><span>New artists</span></button><button class="tab" onclick="showTab('pipeline',this)"><span>Pipeline</span></button><button class="tab" onclick="showTab('scoring',this)"><span>Scoring</span></button></div>
  <div class="panel on" id="panel-top10"><div class="filter-pills" id="top10Filters"><button class="fp on" onclick="filterTop10('all',this)">All platforms</button><button class="fp" onclick="filterTop10('itunes',this)">iTunes</button><button class="fp" onclick="filterTop10('spotify',this)">Spotify</button><button class="fp" onclick="filterTop10('cross',this)">Cross-platform</button><button class="fp" onclick="filterTop10('new',this)">New entrants</button></div><div class="bcard table-card"><div class="table-scroll"><table class="tbl"><thead><tr><th style="width:38px;padding-left:14px">#</th><th style="width:210px" onclick="sortTop10('artist')">Artist</th><th style="width:92px" onclick="sortTop10('oppScore')">Opp. score</th><th style="width:88px" onclick="sortTop10('platform')">Platform</th><th style="width:80px" onclick="sortTop10('peakRank')">Peak rank</th><th style="width:92px" onclick="sortTop10('momentumVal')">Momentum</th><th style="width:78px" onclick="sortTop10('markets')">Signals</th><th style="width:100px">Status</th><th style="width:82px">Action</th></tr></thead><tbody id="top10Body"></tbody></table></div></div></div>
  <div class="panel" id="panel-newartists"><div class="filter-pills" id="newFilters"><button class="fp on" onclick="filterNew('all',this)">All new</button><button class="fp" onclick="filterNew('itunes',this)">iTunes debuts</button><button class="fp" onclick="filterNew('spotify',this)">Spotify debuts</button><button class="fp" onclick="filterNew('highpot',this)">High potential</button></div><div class="bcard table-card"><div class="table-scroll"><table class="tbl"><thead><tr><th style="width:38px;padding-left:14px">#</th><th style="width:210px" onclick="sortNew('artist')">Artist</th><th style="width:88px">Debut</th><th style="width:80px" onclick="sortNew('peakRank')">Peak rank</th><th style="width:100px" onclick="sortNew('streams')">Audience</th><th style="width:88px">Platform</th><th style="width:92px" onclick="sortNew('potential')">Potential</th><th style="width:100px">Recommend</th></tr></thead><tbody id="newArtistBody"></tbody></table></div></div></div>
  <div class="panel" id="panel-pipeline"><div class="two-col" style="margin-bottom:12px"><div class="bcard"><div class="section-hd"><div class="section-title">Active pipeline stages</div></div><div id="pipelineList"></div></div><div class="bcard"><div class="section-hd"><div class="section-title">Stage distribution</div></div><div class="chart-wrap"><canvas id="pipeChart"></canvas></div><div class="leg"><span class="leg-item"><span style="width:10px;height:10px;border-radius:2px;background:#60a5fa;display:inline-block"></span> Prospect</span><span class="leg-item"><span style="width:10px;height:10px;border-radius:2px;background:#fcd34d;display:inline-block"></span> Evaluation</span><span class="leg-item"><span style="width:10px;height:10px;border-radius:2px;background:#c4b5fd;display:inline-block"></span> Negotiation</span><span class="leg-item"><span style="width:10px;height:10px;border-radius:2px;background:#34d399;display:inline-block"></span> Signed</span></div></div></div><div class="bcard"><div class="section-hd"><div class="section-title">Opportunity tiers</div></div><div class="chart-wrap"><canvas id="momChart"></canvas></div></div></div>
...
<div class="spot-bg" id="acqSpot" onclick="closeAcqSpot(event)"><div class="spot-modal" onclick="event.stopPropagation()"><button class="spot-close" onclick="closeAcqSpot()" aria-label="Close">x</button><div class="spot-name" id="asName"></div><div class="spot-sub" id="asSub"></div><div class="spot-grid" id="asKpis"></div><div style="margin-top:4px;display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="asAskBtn" onclick="asAsk()">Full acquisition brief</button><button class="btn" onclick="closeAcqSpot()" style="color:var(--t2)">Close</button></div></div></div>
...
const TOP10=ROWS.slice(0,10),NEW_ARTISTS=ROWS.filter(a=>a.isNew).sort((a,b)=>b.potential-a.potential).slice(0,30),PIPELINE=ROWS.filter(a=>a.oppScore>=60).slice(0,12).map(a=>({...a,since:a.debutDate,value:a.oppScore>=90?'High':a.oppScore>=78?'Medium':'TBD',priority:a.oppScore>=82?'high':'med'}));let top10Filter='all',newFilter='all',top10SortKey='oppScore',top10SortDir=-1,newSortKey='potential',newSortDir=-1,spotArtist='';
...
function filterTop10(f,b){top10Filter=f;if(b){document.querySelectorAll('#top10Filters .fp').forEach(x=>x.classList.remove('on'));b.classList.add('on')}renderTop10()}function sortTop10(k){if(top10SortKey===k)top10SortDir*=-1;else{top10SortKey=k;top10SortDir=-1}renderTop10()}function filterNew(f,b){newFilter=f;if(b){document.querySelectorAll('#newFilters .fp').forEach(x=>x.classList.remove('on'));b.classList.add('on')}renderNewArtists()}function sortNew(k){if(newSortKey===k)newSortDir*=-1;else{newSortKey=k;newSortDir=-1}renderNewArtists()}
function renderTop10(){let d=[...TOP10];if(top10Filter==='itunes')d=d.filter(a=>a.platform==='itunes'||a.platform==='cross');if(top10Filter==='spotify')d=d.filter(a=>a.platform==='spotify'||a.platform==='cross');if(top10Filter==='cross')d=d.filter(a=>a.platform==='cross');if(top10Filter==='new')d=d.filter(a=>a.isNew);d.sort((a,b)=>typeof a[top10SortKey]==='string'?top10SortDir*a[top10SortKey].localeCompare(b[top10SortKey]):top10SortDir*(a[top10SortKey]-b[top10SortKey]));const mx=Math.max(...d.map(a=>a.oppScore),1);document.getElementById('top10Body').innerHTML=d.map((a,i)=>`<tr onclick="openAcqSpotEncoded('${encodeURIComponent(a.artist)}')"><td style="padding-left:14px;font-weight:800;color:var(--t2)">${i+1}</td><td>${artistCell(a)}</td><td><div style="font-weight:800">${a.oppScore}</div><div class="score-bar"><div class="score-fill" style="width:${Math.round(a.oppScore/mx*100)}%;background:${colorForScore(a.oppScore)}"></div></div></td><td>${platformBadge(a.platform)}</td><td style="font-weight:800;text-align:center">${a.peakRank===999?'-':'#'+a.peakRank}</td><td><span style="font-weight:800;color:${a.momentumVal>=0?'var(--green)':'var(--red)'}">${a.momentum}</span></td><td style="text-align:center">${a.markets}</td><td>${stageClass(a.status)?`<span class="pl-stage ${stageClass(a.status)}">${stageLabel(a.status)}</span>`:`<span class="badge b-watch">${stageLabel(a.status)}</span>`}</td><td><button class="btn" onclick="event.stopPropagation();briefArtist('${encodeURIComponent(a.artist)}')">Brief</button></td></tr>`).join('')||'<tr><td colspan="9" style="padding:18px;text-align:center;color:var(--t3)">No targets match this filter.</td></tr>'}
function renderNewArtists(){let d=[...NEW_ARTISTS];if(newFilter==='itunes')d=d.filter(a=>a.platform==='itunes'||a.platform==='cross');if(newFilter==='spotify')d=d.filter(a=>a.platform==='spotify'||a.platform==='cross');if(newFilter==='highpot')d=d.filter(a=>a.potential>=78);d.sort((a,b)=>typeof a[newSortKey]==='string'?newSortDir*a[newSortKey].localeCompare(b[newSortKey]):newSortDir*(a[newSortKey]-b[newSortKey]));document.getElementById('newArtistBody').innerHTML=d.map((a,i)=>`<tr onclick="openAcqSpotEncoded('${encodeURIComponent(a.artist)}')"><td style="padding-left:14px;color:var(--t2);font-weight:800">${i+1}</td><td>${artistCell(a)}</td><td><span class="badge b-new">${esc(a.debutDate)}</span></td><td style="font-weight:800;text-align:center">${a.peakRank===999?'-':'#'+a.peakRank}</td><td>${fmtN(a.streams)}</td><td>${platformBadge(a.platform)}</td><td><div style="display:flex;align-items:center;gap:6px"><span style="font-weight:800;color:${colorForScore(a.potential)}">${a.potential}</span><div class="score-bar" style="flex:1"><div class="score-fill" style="width:${a.potential}%;background:${colorForScore(a.potential)}"></div></div></div></td><td><span class="badge ${recClass(a.recommend)}">${a.recommend}</span></td></tr>`).join('')||'<tr><td colspan="8" style="padding:18px;text-align:center;color:var(--t3)">No new artists match this filter.</td></tr>'}
function renderPipeline(){document.getElementById('pipelineList').innerHTML=PIPELINE.map(p=>`<div class="pipeline-row"><span class="priority-dot" style="background:${p.priority==='high'?'var(--green)':'var(--amber)'}"></span><div style="flex:1;min-width:0"><div style="font-weight:800">${esc(p.artist)}</div><div style="font-size:10px;color:var(--t3)">Since ${esc(p.since)}</div></div><span class="pl-stage ${stageClass(p.status)}">${stageLabel(p.status)}</span><span style="font-size:11px;font-weight:800;min-width:52px;text-align:right">${p.value}</span><button class="btn" onclick="dealUpdateArtist('${encodeURIComponent(p.artist)}')">Update</button></div>`).join('')||'<div style="color:var(--t3);padding:10px 0">No active pipeline targets.</div>'}
function renderCharts(){if(!window.Chart)return;const css=getComputedStyle(document.documentElement),text=css.getPropertyValue('--t2').trim()||'#94a3b8',grid=css.getPropertyValue('--border').trim()||'rgba(148,163,184,.2)',stageCounts=['prospect','eval','nego','signed'].map(s=>PIPELINE.filter(a=>a.status===s).length);new Chart(document.getElementById('pipeChart'),{type:'doughnut',data:{labels:['Prospect','Evaluation','Negotiation','Signed'],datasets:[{data:stageCounts,backgroundColor:['#60a5fa','#fcd34d','#c4b5fd','#34d399'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},cutout:'64%'}});const tiers=[ROWS.filter(a=>a.oppScore>=90).length,ROWS.filter(a=>a.oppScore>=78&&a.oppScore<90).length,ROWS.filter(a=>a.oppScore>=60&&a.oppScore<78).length,ROWS.filter(a=>a.oppScore<60).length];new Chart(document.getElementById('momChart'),{type:'bar',data:{labels:['90+','78-89','60-77','<60'],datasets:[{data:tiers,backgroundColor:['#34d399','#60a5fa','#fcd34d','#fb7185'],borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{grid:{color:grid},ticks:{color:text,precision:0}},x:{grid:{display:false},ticks:{color:text}}}}});new Chart(document.getElementById('scoreChart'),{type:'bar',data:{labels:TOP10.map(a=>a.artist.length>18?a.artist.slice(0,18)+'...':a.artist),datasets:[{label:'iTunes consistency',data:TOP10.map(a=>a.platform==='spotify'?4:Math.min(35,Math.max(10,110-a.peakRank))),backgroundColor:'#60a5fa',stack:'score'},{label:'Spotify reach',data:TOP10.map(a=>a.platform==='itunes'?5:Math.min(30,10+(a.tracks.length*6))),backgroundColor:'#34d399',stack:'score'},{label:'Momentum',data:TOP10.map(a=>Math.max(5,Math.min(20,10+a.momentumVal/2))),backgroundColor:'#fcd34d',stack:'score'},{label:'Cross-platform',data:TOP10.map(a=>a.platform==='cross'?15:0),backgroundColor:'#c4b5fd',stack:'score'}]},options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',plugins:{legend:{position:'bottom',labels:{color:text,boxWidth:10}}},scales:{x:{stacked:true,max:100,grid:{color:grid},ticks:{color:text}},y:{stacked:true,grid:{display:false},ticks:{color:text}}}}})}
...
renderKpis();renderTop10();renderNewArtists();renderPipeline();renderCharts();
</script></body></html>
""".replace("__PAYLOAD__", data_json).replace("__THEME__", theme_css)
