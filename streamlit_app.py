...
if not st.session_state.chat_open:
    if st.button("💬", help="Chat with AI"):
        st.session_state.chat_open = True
        st.rerun()
else:
    chat_box = st.container()
    with chat_box:
        st.markdown('<div id="chatbot-container-marker"></div>', unsafe_allow_html=True)
        
        # Custom Header & Close Button Row
        col1, col2 = st.columns([0.85, 0.15])
        with col1:
            st.markdown("""
                <div style="display: flex; align-items: center; gap: 10px; color: white;">
                    <div style="background: linear-gradient(135deg, #7c5cfc, #4f8ef7); width: 32px; height: 32px; border-radius: 50%; display:flex; align-items:center; justify-content:center; font-size: 16px;">🤖</div>
                    <h3 style="margin: 0; font-size: 17px; font-weight: 600; color: white; letter-spacing: 0.5px;">AI Artist Assistant</h3>
                </div>
            """, unsafe_allow_html=True)
        with col2:
        ...
        if "messages" not in st.session_state:
            st.session_state.messages = [
                {"role": "assistant", "content": "Hello! I'm your Artist 360 AI. Ask me about the dashboard data!"}
            ]
        ...
        if prompt := st.chat_input("Ask about chart trends..."):
            ...
            with msg_container.chat_message("assistant"):
                try:
                    ...
                    try:
                        ...
                        itunes_csv = dash_data["itunes"].head(10).to_csv(index=False)
                        spotify_csv = dash_data["spotify"].head(10).to_csv(index=False)
                        live_data_context = f"\n\n--- LIVE DASHBOARD DATA ---\nTop 10 iTunes:\n{itunes_csv}\nTop 10 Spotify:\n{spotify_csv}\n(Use this live data if the user asks about the dashboard's current top artists, rankings, or listeners!)"
                    except Exception:
                        live_data_context = ""

                    sys_msg = f"""You are an expert AI music analyst for the 'Artist 360 Intelligence' dashboard. 
When asked about an artist, you MUST format your response exactly like this rich, detailed Markdown Profile:

# 🎤 [Artist Name] — Artist Profile
[A short biography paragraph with bold key terms like Wikipedia].

### 🌟 Career Highlights
* [Bullet points with bold terms for achievements and records]
...
### 🏆 Awards & Accolades
* [Specific bullet points about Grammys, Billboard, etc.]

### 🌎 Tours
* [Bullet points of highest grossing or famous tours]

### 🎸 Instruments & Genres
* **Genres:** [Genres]
* **Instruments:** [Instruments]

Always end with a friendly question asking what else they want to explore, accompanied by a 😊 emoji. Make it highly structured, engaging, and professional!

CRITICAL INSTRUCTIONS FOR EDGE CASES:

1. STRICT TOPIC RESTRICTION & ACTORS DENIAL (Rule 1):
If the user asks about a politician, historical figure, or an actor who is NOT known for a major music career (for example: Allu Arjun, Salman Khan, Gandhi), you MUST politely decline the request. (Do not create a profile just because an actor did one playback song). You MUST use EXACTLY this Markdown format to deny:

I'm sorry, but I'm **Artist 360 Intelligence**, an AI agent specialized exclusively in **music artists, their profiles, discographies, tours, collaborations, and related information**.

🎵 **[Insert Subject Name]** does not appear to be a recognized mainstream music artist in my knowledge domain.
___
**Here's what I *can* help you with:**
- 🎤 **Artist Profiles** — Biography, background, and career overview
- 💿 **Discography** — Albums, singles, and EPs
- 🎶 **Top Tracks** — Most popular or latest songs
- 🤝 **Collaborations** — Featured artists and duets
- 🌍 **Tours & Events** — Concert tours and live performances
- 🏆 **Awards & Achievements** — Grammy wins, nominations, and more

2. MULTI-HYPHENATES WITH REAL MUSIC CAREERS (Rule 2):
ONLY if the requested person is an actor/public figure who HAS released official mainstream international singles or albums (like Priyanka Chopra, Selena Gomez), do not decline. Instead:
   - add `**Note:** [Explain they are known as an actor but have an established music career]` right below the Title.
   - Use `### 👤 Basic Info` for general details.
   - Use `### 🎵 Music Career` for their music summary.
...
components.html(
    ...
    setInterval(function() {
        const marker = window.parent.document.getElementById('chatbot-container-marker');
        if (marker) {
            let elementContainer = marker.closest('div[data-testid="stElementContainer"]');
            if (elementContainer && elementContainer.parentElement) {
                let targetBox = elementContainer.parentElement;
                // Target box is the inside of the main st.container()
                // Let's actually style its parent wrapper to properly float the WHOLE st.container()
                let outerWrapper = targetBox;
                if(targetBox.parentElement && targetBox.parentElement.getAttribute('data-testid') === 'stVerticalBlock') {
                     outerWrapper = targetBox.parentElement;
                }
                
                outerWrapper.style.setProperty('position', 'fixed', 'important');
                outerWrapper.style.setProperty('bottom', '24px', 'important');
                outerWrapper.style.setProperty('right', '24px', 'important');
                outerWrapper.style.setProperty('width', '400px', 'important');
                outerWrapper.style.setProperty('z-index', '999999', 'important');
                outerWrapper.style.setProperty('background', '#12182a', 'important');
                // Create gradient border effect
                outerWrapper.style.setProperty('border', '2px solid #4f8ef7', 'important');
                outerWrapper.style.setProperty('border-radius', '16px', 'important');
                outerWrapper.style.setProperty('box-shadow', '0 12px 32px rgba(0,0,0,0.8)', 'important');
                outerWrapper.style.setProperty('padding', '20px', 'important');
                
                // Also hide standard margins
                outerWrapper.style.setProperty('margin', '0', 'important');
            }
        }
        ...
        buttons.forEach(btn => {
            if (btn.innerText.includes('💬')) {
                ...
                if (container) {
                    ...
                    container.style.setProperty('width', '65px', 'important');
                    container.style.setProperty('height', '65px', 'important');
                    
                    btn.style.setProperty('border-radius', '50%', 'important');
                    btn.style.setProperty('width', '100%', 'important');
                    btn.style.setProperty('height', '100%', 'important');
                    btn.style.setProperty('background', 'linear-gradient(135deg, #4f8ef7, #7c5cfc)', 'important');
                    btn.style.setProperty('box-shadow', '0 10px 25px rgba(79,142,247,0.5)', 'important');
                    btn.style.setProperty('border', 'none', 'important');
                    
                    const p = btn.querySelector('p');
                    if (p) {
                        p.style.setProperty('font-size', '32px', 'important');
                    ...
                    btn.onmouseover = function() {
                        ...
                        this.style.setProperty('box-shadow', '0 15px 35px rgba(79,142,247,0.6)', 'important');
                    }
                    btn.onmouseout = function() {
                        this.style.setProperty('transform', 'scale(1.0)', 'important');
                        this.style.setProperty('box-shadow', '0 10px 25px rgba(79,142,247,0.5)', 'important');
                    }
                }
            }
        });
